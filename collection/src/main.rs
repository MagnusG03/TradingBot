...
fn build_client() -> Client {
    ...
    headers.insert(
        USER_AGENT,
        HeaderValue::from_str(&sec_user_agent)
            .unwrap_or_else(|_| HeaderValue::from_static("TradingBot admin@example.com")),
    );
    headers.insert(
...
fn sentiment_analysis(text: &str) -> f64 {
    ...
    fn word_score(token: &str) -> Option<f64> {
        Some(match token {
            "acquire" | "acquires" | "acquired" | "approval" | "approvals" | "approve"
            | "approved" | "beat" | "beats" | "bullish" | "exceed" | "exceeds" | "exceeded"
            | "growth" | "grow" | "grows" | "grew" | "improve" | "improves" | "improved"
            | "outperform" | "outperformed" | "profit" | "profits" | "profitable" | "rebound"
            | "recovery" | "strong" | "surge" | "surges" | "surged" | "upgrade" | "upgrades"
            | "upgraded" | "win" | "wins" | "won" => 1.0,
            "boost" | "boosts" | "gain" | "gains" | "gained" | "jump" | "jumps" | "jumped"
            | "lift" | "lifts" | "lifted" | "raise" | "raises" | "raised" => 0.8,
            "bankruptcy" | "bankrupt" | "bearish" | "breach" | "concern" | "concerns"
            | "decline" | "declines" | "declined" | "delay" | "delays" | "delayed"
            | "downgrade" | "downgrades" | "downgraded" | "drop" | "drops" | "dropped" | "fall"
            | "falls" | "fell" | "fraud" | "investigation" | "investigations" | "lawsuit"
            | "lawsuits" | "loss" | "losses" | "miss" | "misses" | "missed" | "plunge"
            | "plunges" | "plunged" | "probe" | "probes" | "recall" | "recalls" | "risk"
            | "risks" | "scandal" | "slump" | "slumps" | "slumped" | "weak" | "warning"
            | "warnings" => -1.0,
    ...
    fn phrase_score(tokens: &[&str], index: usize) -> Option<(usize, f64)> {
        ...
        match (first, second) {
            ...
            (first, Some(second))
                if matches_any(first, &["data", "cyber"]) && second == "breach" =>
            {
                Some((2, -1.8))
            }
            (first, Some(second))
            ...
            (first, Some(second))
                if matches_any(first, &["trade", "trading"]) && second == "halt" =>
            {
                Some((2, -1.8))
            }
            (first, Some(second))
            ...
            (first, Some(second))
                if matches_any(first, &["stock", "shares"])
                    && matches_any(
                        second,
                        &[
                            "fall", "falls", "drop", "drops", "slump", "slumps", "plunge",
                            "plunges",
                        ],
                    ) =>
            {
    ...
    if let Some(score) = clause_adjusted_sentiment(&tokens) {
        return score;
    }

    score_token_slice(&tokens).normalized().unwrap_or(0.0)
}
...
fn average_article_sentiment(articles: &[GoogleArticle]) -> f64 {
    if articles.is_empty() {
        return 0.0;
    }
...
async fn gather_data(ticker: &str) -> MLData {
    ...
    let general_market_sentiment_1d = average_article_sentiment(&market_articles);
    let general_sector_sentiment_1d = average_article_sentiment(&sector_articles);
